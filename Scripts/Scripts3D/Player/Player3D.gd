...
func _process(delta):
	...
	if (rotatableSprites.has(animator.current_animation)):
		...
		if abs(spriteRotation-90) >= 32 or rotation2d != gravityAngle or !ground:
			spriteRotationToSet2d = deg_to_rad(snapped(spriteRotation,45)-90)-rotation2d-gravityAngle
		else:
...
func _physics_process(delta):
	...
	if (ground):
		groundSpeed = movement2d.x
		
	# wall detection
	# viv todo: this needs to be rewritten for 3d
	...
	if (camera != null):
		...
		# viv todo: don't completely wing this
		var viewSize = Rect2(0, 0, 640, 480).size #get_viewport_rect().size
	...
	if centerReference != null:
		if centerReference.position != Vector3.ZERO:
...
func set_state(newState, forceMask = Vector3.ZERO):
	...
	var forcePoseChange = Vector3.ZERO
	
	if (forceMask == Vector3.ZERO):
		match(newState):
			...
			STATES.SPINDASH:
				# change hitbox size
				$HitBox.shape.size = hitbox_to_3d(currentHitbox.CROUCH)
...
func hitbox_to_3d(hitbox2d):
	return Vector3(hitbox2d.x, hitbox2d.y, hitbox2d.x)
...
func hit_player(damagePoint = global_position, damageType = 0, soundID = 6):
	...
	if (currentState != STATES.HIT and invTime <= 0 and supTime <= 0 and (shieldSprite.get_node("InstaShieldHitbox/HitBox").disabled or character != CHARACTERS.SONIC)):
		movement2d.x = sign(global_position.x-damagePoint.x)*2*60
		movement2d.y = -4*60
		if (movement2d.x == 0):
			movement2d.x = 2*60
		# check for water
		if water:
			movement2d = movement*0.5
...
func respawn():
	if partner != null:
		...
		switch_physics()
		#global_position = partner.global_position+Vector2(0,-get_viewport_rect().size.y) # viv todo: fix partner respawn
	...
	if inputs[INPUTS.XINPUT] != 0:
		...
		if movement2d.x*inputs[INPUTS.XINPUT] < top:
			...
			if sign(movement2d.x) == inputs[INPUTS.XINPUT] or sign(movement2d.x) == 0:
				if abs(movement2d.x) < top:
					movement2d.x = move_toward(movement2d.x,top*inputs[INPUTS.XINPUT],acc/GlobalFunctions.div_by_delta(delta))
			else:
				# reverse direction
				movement2d.x += dec/GlobalFunctions.div_by_delta(delta)*inputs[INPUTS.XINPUT]
				# implament weird turning quirk
				if (sign(movement2d.x) != sign(movement2d.x-dec/GlobalFunctions.div_by_delta(delta)*inputs[INPUTS.XINPUT])):
					movement2d.x = 0.5*60*sign(movement2d.x)
	else:
		# come to a stop if neither left or right is pressed
		if (movement2d.x != 0):
			# check that decreasing movement won't go too far
			if (sign(movement2d.x - (frc/GlobalFunctions.div_by_delta(delta))*sign(movement2d.x)) == sign(movement2d.x)):
				movement2d.x -= (frc/GlobalFunctions.div_by_delta(delta))*sign(movement2d.x)
			else:
				movement2d.x -= movement2d.x
...
func action_water_run_handle():
	...
	dash.scale.x = sign(movement2d.x)
...
func handle_animation_speed(gSpeed = groundSpeed):
	match(animator.current_animation):
		...
		"climb":
			animator.speed_scale = -movement2d.y/(40.0*(1.0+float(isSuper)))
		_:
			animator.speed_scale = 1
