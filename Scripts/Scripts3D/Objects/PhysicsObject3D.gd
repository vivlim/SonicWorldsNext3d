...
var maxCharGroundHeight = 16 * pixelSize # this is to stop players getting stuck at the bottom of 16x16 tiles, 
...
var groundLookDistance = 14 * pixelSize # how far down to look
@onready var pushRadius = max(($HitBox.shape.size.x/2)+1,10*pixelSize) # original push radius is 10
...
var ground = true
var roof = false
var moveStepLength = 8*60 * pixelSize
...
@export var pixelObjectStep = 4 * pixelSize

func _ready():
	slopeCheck.debug_shape_custom_color = Color.BLUE_VIOLET
...
func update_sensors():
	...
	slopeCheck.global_rotation = rotationSnap3d
...
func _physics_process(delta):
	...
	while (!moveRemaining.is_equal_approx(Vector3.ZERO) or checkOverride) and !translate:
		checkOverride = false
	...
	position2d = Translate3DTo2D(position)
